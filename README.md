...
이 프로젝트는 AI를 활용하되, Spring Boot 백엔드 개발 역량을 직접 키우는 것을 목표로 합니다.
AI가 만들어준 결과를 그대로 제출하는 방식이 아니라, 설명을 듣고 이해한 뒤 팀원이 직접 구현하는 방식으로 진행합니다.

1. AI는 설명과 예제 용도로 사용합니다.
2. 핵심 비즈니스 로직은 직접 구현합니다.
3. 이해하지 못한 코드는 PR하지 않습니다.
4. AI가 생성한 코드는 복사해서 붙여넣지 않고 직접 타이핑합니다.
5. 구현한 기능은 금요일 회의에서 직접 설명합니다.
6. 구현 과정과 새롭게 배운 내용은 TIL로 작성합니다.
...
- [프로젝트 일정](docs/wbs/project-schedule.md)
- [페이지 스펙](docs/wbs/page-spec.md)
- [MVP 기능 범위](docs/wbs/mvp-scope.md)
- [API 명세](docs/architecture/api-spec.md)
- [DB 테이블 명세](docs/architecture/database-schema.md)
